...
- Revert target rule: interruptions of active work (`alert`/`error`/`compact`) pass `@prev` to `schedule_revert` so the session resumes the last stable state (`thinking`/`idle`) captured in `prev_state`. End-of-task or greeting states (`done`/`start`/`permission`) pass nothing — default `idle`. `prev_state` is captured in `mutate update`'s `TRANSIENT` branch; carries forward across chained transients (error → alert keeps thinking). When adding a new revertable state, decide which bucket it belongs to and wire the case in `show()` accordingly.
